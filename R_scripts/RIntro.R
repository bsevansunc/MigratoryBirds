...
# R has easy ways to get help. In the space below, type the function "getwd" with 
# no parentheses and then the tab button on your keyboard. This will bring up
# a help window that describes the function. You can also use the help tab
# on the lower left hand window of RStudio.
...
# MigratoryBirds folder on your computer.
  ...
  setwd(<Your working directory address here>)
...
# 6. Loading the data into R: We will now download the file "encounters.csv" into R.
...
# such as:
  ...
  encounters2 <- encounters # Creates a new file named encounters2 from encounters

# -or-

  encounters -> encounters2 # As above, just in the other direction

# Take a look at the history tab of the right-hand pane in RStudio, you will see 
...
# To view the types (eg., numeric, integer, facor, character) of variables 
...
# Challenge: Change the name of just the new column to "alhpa.code"
...
tapply(encounters[encounters$species == 'GRCA','wing'],
       encounters[encounters$species == 'GRCA','sex'],mean)

# Well that's a lot of script to write! Let's use aggregate to see average
# wing chords by species and sex:

aggregate(encounters$wing, 
          by = list(encounters$species, encounters$sex), mean)
...
# you can change this by resetting the levels:
...
# 2. A quick internet search using the search terms "r cran change case", 
# provided another way to recode this variable. See if you can find out how 
# to do this and change lower case "f" records to "F" records and write 
# your code in the space below!
...
# Note: "na.rm = T" removes NA records, which is necessary to calculate 
# means.
...
# We could also combine the above statement into one, as:
  ...
  amro$weight.class = ifelse(amro$mass > mean(amro$mass, na.rm = T),
                             'heavy','light')



  encounters$sex = ifelse(encounters$sex == 'm','M',encounters$sex)
...
# on multiple lines, which I feel helps display what is being done):
  ...
  encounters$sex = ifelse(encounters$sex == 'm','M',
                          ifelse(encounters$sex == 'u','U',
                                 encounters$sex))

# Challenge: Using one of the three methods above, for each species determine
# the average body mass by the following HY and AHY classes (hint: data 
# subsetting and recoding is necessary!).
...
# We can determine the standard deviation of wing chord length as:
  ...
  sd(na.omit(cach.wing))

# Note: na.omit is a function that removes NA values from the vector.

# We can determine the number of records as:

  length(na.omit(cach.wing))

# We can calculate the standard error as:

  sd(na.omit(cach.wing))/sqrt(length(na.omit(cach.wing)))
...
# Challenge 1! Write a function that can calculate and output the mean
# and standard error for any species within the encounters frame.

# Challenge 2! Write a single function that can be used to read in the
# original encounters dataset and recode all lower case names to upper,
# recode any mispellings (e.g., non-alpha code species names), and 
# remove any blanks. Here's a start:

  encounter.clean = function(encounters.csv){
    <YOUR FUNCTION HERE>
    }

# To execture your function:

  encounters = encounter.clean('Data/encounters.csv')
...
fibonacci.sequence = c(0,1)
for(i in 2:10){
  fibonacci.sequence[i] = fibs[i-1]+fibs[i-2]
}
...
for (i in 1:length(sp.list)){
  df[i,1] = sp.list[i]
  df[i,2] = mean(na.omit(df.species.sub(i)))
...
age.wings = function(age.class){
  ...
  # For loop:
    for (i in 1:length(sp.list)){
      df[i,1] = sp.list[i]
      df[i,2] = mean(df.species.sub(i), na.rm = T)
      df[i,3] = se(df.species.sub(i))
    }
